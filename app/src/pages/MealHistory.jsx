  ...
  return (
    <div style={{ maxWidth: 1024, margin: '0 auto', paddingBottom: '5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <div style={{ backgroundColor: '#f3f4f6', padding: '0.75rem', borderRadius: '1rem' }}>
          <HistoryIcon style={{ width: 32, height: 32, color: '#4b5563' }} />
        </div>
        <div>
          <h1 style={{ fontSize: '2.25rem', fontWeight: 700, color: '#1a1a1a', marginBottom: '0.25rem' }}>Meal History</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          const dateStr = new Date(plan.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: '2-digit', minute: '2-digit' });
          return (
            <div key={plan.id} style={{ backgroundColor: 'white', borderRadius: '1rem', padding: '1.5rem', boxShadow: '0 1px 2px rgb(0 0 0 / 0.05)', border: '1px solid #e5e5e5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                  ...
                  <div>
                    <h3 style={{ fontSize: '1.125rem', fontWeight: 700, color: '#1a1a1a', marginBottom: '0.25rem' }}>{plan.duration}-Day Plan</h3>
                    <p style={{ color: '#6b7280', fontSize: '0.875rem' }}>{dateStr} • {plan.duration * 3} Meals</p>
                  </div>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <button onClick={() => handleReuse(plan)} onMouseEnter={() => setReuseHovered((p) => ({ ...p, [plan.id]: true }))} onMouseLeave={() => setReuseHovered((p) => ({ ...p, [plan.id]: false }))} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1.25rem', backgroundColor: reuseHovered[plan.id] ? '#e4002b' : '#f5f5f5', color: reuseHovered[plan.id] ? 'white' : '#e4002b', border: '1px solid rgba(228, 0, 43, 0.2)', borderRadius: '0.5rem', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s', fontSize: '0.875rem' }}>
                    <RefreshCwIcon style={{ width: 16, height: 16 }} /> Reuse
                  </button>
                  <button onClick={() => deleteFromHistory(plan.id)} onMouseEnter={() => setDeleteHovered((p) => ({ ...p, [plan.id]: true }))} onMouseLeave={() => setDeleteHovered((p) => ({ ...p, [plan.id]: false }))} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', padding: '0.5rem 1rem', backgroundColor: deleteHovered[plan.id] ? '#fef2f2' : 'transparent', color: deleteHovered[plan.id] ? '#b91c1c' : '#9ca3af', border: '1px solid transparent', borderRadius: '0.5rem', cursor: 'pointer', transition: 'all 0.2s', fontSize: '0.875rem' }}>
                    <Trash2Icon style={{ width: 16, height: 16 }} /> Delete
                  </button>
                </div>
              </div>);
