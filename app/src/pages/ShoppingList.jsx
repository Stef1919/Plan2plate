...
export function ShoppingList() {
  ...
  const handleExport = () => {
    const text = Object.entries(groupedList).map(([category, items]) => {
  ...
  return (
    <div style={{ maxWidth: 896, margin: '0 auto', paddingBottom: '5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '2rem' }}>
        <div>
          <h1 style={{ fontSize: '2.25rem', fontWeight: 700, color: '#111827', marginBottom: '0.5rem' }}>Shopping List</h1>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            style={{
              ...
              backgroundColor: exportHovered ? '#f9fafb' : 'white',
      ...
      <div style={{ textAlign: 'center', padding: '8rem 0', backgroundColor: 'white', borderRadius: '1.5rem', border: '2px dashed #d1d5db' }}>
          ...
          style={{
            backgroundColor: planHovered ? '#d66229' : '#e8723a',
            color: 'white',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        ...
        <motion.div
          ...
          key={category}
          style={{ backgroundColor: 'white', borderRadius: '1rem', padding: '1.5rem', boxShadow: '0 1px 2px rgb(0 0 0 / 0.05)', border: '1px solid #f3f4f6' }}>
          
              <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#4a6741', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem', borderBottom: '1px solid #f3f4f6', paddingBottom: '0.75rem' }}>
                {category}
                <span style={{ fontSize: '0.875rem', fontWeight: 400, color: '#9ca3af', backgroundColor: '#f3f4f6', padding: '0.125rem 0.5rem', borderRadius: '9999px' }}>
                  {items.length}
                </span>
              </h3>
              ...
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  style={{ width: 20, height: 20, accentColor: '#4a6741' }} />
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                      <span style={{ color: '#6b7280', backgroundColor: '#f9fafb', padding: '0.25rem 0.75rem', borderRadius: '0.5rem', fontWeight: 500 }}>
                        {item.amount} {item.unit}
                      </span>
                      <button
                  onClick={() => removeFromShoppingList(item.id)}
                  style={{
                    color: hoveredItems[item.id] ? '#ef4444' : '#d1d5db',
                    padding: '0.25rem',
                    backgroundColor: 'transparent',
                    border: 'none',
                    cursor: 'pointer',
                    opacity: hoveredItems[item.id] ? 1 : 0,
                    transition: 'all 0.2s'
                  }}>
