  ...
  return (
    <div style={{ maxWidth: 1024, margin: '0 auto', paddingBottom: '5rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
        <div style={{ backgroundColor: '#fff7ed', padding: '0.75rem', borderRadius: '1rem' }}>
          <BookmarkIcon style={{ width: 32, height: 32, color: '#c2410c' }} />
        </div>
        <div>
          <h1 style={{ fontSize: '2.25rem', fontWeight: 700, color: '#111827', marginBottom: '0.25rem' }}>Saved Plans</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          return (
            <div key={plan.id} style={{ backgroundColor: 'white', borderRadius: '1rem', boxShadow: '0 1px 2px rgb(0 0 0 / 0.05)', border: '1px solid #f3f4f6', overflow: 'hidden' }}>
                <div
                ...
                onClick={() => toggleExpand(plan.id)}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
                    ...
                    <div>
                      <h3 style={{ fontSize: '1.125rem', fontWeight: 700, color: '#111827', marginBottom: '0.25rem' }}>Saved on {dateStr}</h3>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <button
                    onClick={(e) => {e.stopPropagation();handleReuse(plan);}}
                    ...
                    <button
                    onClick={(e) => {e.stopPropagation();deleteSavedPlan(plan.id);}}
              ...
              <div style={{ padding: '0 1.5rem 1.5rem', borderTop: '1px solid #f3f4f6' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(plan.duration, 7)}, 1fr)`, gap: '0.75rem', paddingTop: '1rem' }}>
                      {plan.days.map((day, i) =>
                  <div key={i} style={{ backgroundColor: '#faf8f5', borderRadius: '0.75rem', padding: '0.75rem', fontSize: '0.8rem' }}>
                          <p style={{ fontWeight: 700, color: '#4a6741', marginBottom: '0.5rem', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Day {i + 1}</p>
